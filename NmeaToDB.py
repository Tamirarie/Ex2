...
def nmeaRMC(INPUT):
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS nmeaRMC
...
def nmeaGGA(INPUT):
    ...
    c = conn.cursor()
    # Create table
    c.execute('''CREATE TABLE IF NOT EXISTS nmeaGGA
    ...
    with open(INPUT, 'r') as input_file:
        ...
        for row in reader:
            ...
            if not row[0].startswith('$GPGGA'):
                ...
                # convert the Python datetime into your preferred string format, see http://www.tutorialspoint.com/python/time_strftime.htm for futher possibilities
                date_and_time = date_and_time.strftime('%H:%M:%S.%f')[:-3] # [:-3] cuts off the last three characters (trailing zeros from the fractional seconds)
                ...
                # Insert a row of data
                c.execute("INSERT INTO nmeaGGA VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
...
INPUT = 'NMEAfiles\stockholm_walk.nmea'
nmeaGGA(INPUT)
##nmeaRMC(INPUT)
